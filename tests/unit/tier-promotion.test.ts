...
describe("Tier Promotion", () => {
  ...
  describe("isTierHigherThan", () => {
    ...
    it("working is NOT > semantic", () => expect(isTierHigherThan("working", "semantic")).toBe(false));
    it("same tier is NOT higher", () => expect(isTierHigherThan("semantic", "semantic")).toBe(false));
  });
  ...
  describe("canPromoteWorkingToEpisodic", () => {
    ...
    it("rejects quarantined working entries", () => {
      const entry = makeEntry({
        ...
        trust: { status: "quarantine", score: 0, validatedAt: null, expiresAt: new Date("2026-04-01") },
      });
      expect(canPromoteWorkingToEpisodic(entry)).toBe(false);
    });
    ...
    it("rejects stale working entries", () => {
      const entry = makeEntry({
        id: "1",
        consolidationTier: "working",
        trust: { status: "stale", score: 0.3, validatedAt: new Date(), expiresAt: new Date("2026-04-01") },
      });
      expect(canPromoteWorkingToEpisodic(entry)).toBe(false);
    });
  });
  ...
  describe("canPromoteEpisodicToSemantic", () => {
    it("promotes validated episodic entries with access", () => {
      const entry = makeEntry({ id: "1", consolidationTier: "episodic", accessCount: 2 });
      expect(canPromoteEpisodicToSemantic(entry)).toBe(true);
    });

    it("rejects episodic entries with zero access", () => {
      const entry = makeEntry({ id: "1", consolidationTier: "episodic", accessCount: 0 });
      expect(canPromoteEpisodicToSemantic(entry)).toBe(false);
    });

    it("rejects non-episodic entries", () => {
      const entry = makeEntry({ id: "1", consolidationTier: "working", accessCount: 5 });
      expect(canPromoteEpisodicToSemantic(entry)).toBe(false);
    });
