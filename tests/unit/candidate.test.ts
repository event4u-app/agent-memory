...
describe("Auto-Classification", () => {
	...
	it("assigns tier based on source", () => {
		expect(classifyCandidate(makeCandidate({ source: "observation" })).consolidationTier).toBe(
			"working",
		);
		expect(classifyCandidate(makeCandidate({ source: "session-summary" })).consolidationTier).toBe(
			"episodic",
		);
		expect(classifyCandidate(makeCandidate({ source: "file-scanner" })).consolidationTier).toBe(
			"semantic",
		);
		expect(classifyCandidate(makeCandidate({ source: "git-reader" })).consolidationTier).toBe(
			"semantic",
		);
		expect(classifyCandidate(makeCandidate({ source: "manual" })).consolidationTier).toBe(
			"semantic",
		);
	});
