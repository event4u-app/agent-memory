...
describe("Privacy Audit", () => {
  ...
  it("preserves non-sensitive content", () => {
    const input = "The architecture decision was to use PostgreSQL for persistence. The module structure follows hexagonal architecture.";
    const result = applyPrivacyFilter(input);
    expect(result).toBe(input);
  });

  it("handles mixed sensitive and non-sensitive content", () => {
    const input = 'Use the api_key="sk-proj-abc123456789abcdef1234567890" to connect. The endpoint is /api/v1/memories.';
    const result = applyPrivacyFilter(input);
...
describe("Access Scope Validation", async () => {
  const { validateScope, canAccess, filterByScope } = await import("../../src/security/access-scope.js");

  it("rejects empty repository", () => {
    const result = validateScope({ repository: "", files: [], symbols: [], modules: [] });
    expect(result.valid).toBe(false);
    expect(result.errors).toContain("repository is required");
  });

  it("rejects absolute file paths", () => {
    const result = validateScope({ repository: "test", files: ["/etc/passwd"], symbols: [], modules: [] });
    expect(result.valid).toBe(false);
  });

  it("rejects path traversal", () => {
    const result = validateScope({ repository: "test", files: ["../../secret.txt"], symbols: [], modules: [] });
    expect(result.valid).toBe(false);
  });

  it("accepts valid scope", () => {
    const result = validateScope({ repository: "my-repo", files: ["src/index.ts"], symbols: ["MyClass"], modules: ["core"] });
