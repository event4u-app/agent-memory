...
import { isExpired, findExpiredEntries, applyExpiryFilter } from "../../src/trust/expiry.js";
  ...
  describe("isExpired", () => {
    ...
    it("returns false when expires_at is in the future", () => {
      const entry = makeEntry({
        id: "1",
        trust: { status: "validated", score: 0.8, validatedAt: new Date("2026-01-01"), expiresAt: new Date("2026-04-01") },
      });
    ...
    it("returns true when expires_at equals now (boundary)", () => {
      const entry = makeEntry({
        id: "1",
        trust: { status: "validated", score: 0.8, validatedAt: new Date("2026-01-01"), expiresAt: now },
      });
  ...
  describe("findExpiredEntries", () => {
    it("returns IDs of expired validated entries", () => {
      const entries = [
        ...
        makeEntry({
          id: "fresh-1",
          trust: { status: "validated", score: 0.8, validatedAt: new Date(), expiresAt: new Date("2026-04-01") },
        }),
    ...
    it("skips entries in statuses that cannot transition to stale", () => {
      const entries = [
        makeEntry({
          id: "quarantined",
          trust: { status: "quarantine", score: 0, validatedAt: null, expiresAt: new Date("2026-01-01") },
        }),
        makeEntry({
          id: "rejected",
          trust: { status: "rejected", score: 0, validatedAt: null, expiresAt: new Date("2026-01-01") },
        }),
        makeEntry({
          id: "archived",
          trust: { status: "archived", score: 0, validatedAt: null, expiresAt: new Date("2026-01-01") },
        }),
      ];
      const ids = findExpiredEntries(entries, now);
      expect(ids).toEqual([]);
    ...
    it("includes already-stale expired entries that can re-stale", () => {
      ...
      const entries = [
        makeEntry({
          id: "stale-1",
          trust: { status: "stale", score: 0.4, validatedAt: new Date("2025-01-01"), expiresAt: new Date("2026-01-01") },
        }),
      ];
      const ids = findExpiredEntries(entries, now);
    ...
    it("returns empty array when no entries are expired", () => {
      const entries = [
        makeEntry({
          id: "fresh",
          trust: { status: "validated", score: 0.9, validatedAt: new Date(), expiresAt: new Date("2026-12-31") },
        }),
      ];
      const ids = findExpiredEntries(entries, now);
      expect(ids).toEqual([]);
  ...
  describe("applyExpiryFilter", () => {
    it("separates servable from stale entries", () => {
      const entries = [
        makeEntry({
          id: "fresh",
          trust: { status: "validated", score: 0.9, validatedAt: new Date(), expiresAt: new Date("2026-04-01") },
        }),
        makeEntry({
          id: "stale",
          trust: { status: "stale", score: 0.4, validatedAt: new Date("2025-01-01"), expiresAt: new Date("2026-01-01") },
    ...
    it("filters out non-servable statuses", () => {
      const entries = [
        makeEntry({ id: "q", trust: { status: "quarantine", score: 0, validatedAt: null, expiresAt: new Date("2026-04-01") } }),
        makeEntry({ id: "i", trust: { status: "invalidated", score: 0, validatedAt: null, expiresAt: new Date("2026-04-01") } }),
        makeEntry({ id: "r", trust: { status: "rejected", score: 0, validatedAt: null, expiresAt: new Date("2026-04-01") } }),
        makeEntry({ id: "p", trust: { status: "poisoned", score: 0, validatedAt: null, expiresAt: new Date("2026-04-01") } }),
        makeEntry({ id: "a", trust: { status: "archived", score: 0, validatedAt: null, expiresAt: new Date("2026-04-01") } }),
      ];
      const result = applyExpiryFilter(entries, now);
      expect(result.servable).toHaveLength(0);
    ...
    it("handles mixed bag correctly", () => {
      const entries = [
        makeEntry({ id: "valid-fresh", trust: { status: "validated", score: 0.9, validatedAt: new Date(), expiresAt: new Date("2026-12-31") } }),
        makeEntry({ id: "valid-expired" }), // expires Feb 1
        makeEntry({ id: "already-stale", trust: { status: "stale", score: 0.3, validatedAt: new Date("2025-01-01"), expiresAt: new Date("2025-06-01") } }),
        makeEntry({ id: "quarantined", trust: { status: "quarantine", score: 0, validatedAt: null, expiresAt: new Date("2026-04-01") } }),
        makeEntry({ id: "poisoned", trust: { status: "poisoned", score: 0, validatedAt: null, expiresAt: new Date("2026-04-01") } }),
      ];
      const result = applyExpiryFilter(entries, now);
      expect(result.servable).toHaveLength(1);
