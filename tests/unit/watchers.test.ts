...
function makeEntry(files: string[], symbols: string[] = [], modules: string[] = []): MemoryEntry {
  return {
    ...
    trust: { status: "validated", score: 0.8, validatedAt: new Date(), expiresAt: new Date("2026-06-01") },
    embeddingText: "test",
    embedding: null,
    accessCount: 0,
    lastAccessedAt: null,
...
function makeChange(filePath: string, added = 10, deleted = 5, extra: Partial<FileChange> = {}): FileChange {
  return { filePath, linesAdded: added, linesDeleted: deleted, isNew: false, isDeleted: false, isRenamed: false, ...extra };
}
...
describe("matchFileWatches", () => {
  ...
  it("detects renamed files that were being watched", () => {
    ...
    const changes = [makeChange("src/new-name.ts", 0, 0, { isRenamed: true, oldPath: "src/old-name.ts" })];
