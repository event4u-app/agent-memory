...
export function rrfFuse(
	...
	const fusedScores = new Map<string, { score: number; streamScores: Record<string, number> }>();
	...
	const results: FusionResult[] = Array.from(fusedScores.entries()).map(([id, data]) => ({
		id,
		score: data.score,
	...
	logger.debug(
		{
			...
			totalCandidates: fusedScores.size,
			limit,
		},
