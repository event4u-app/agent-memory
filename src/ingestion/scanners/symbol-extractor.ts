 ...
 */
export async function extractSymbols(filePath: string, root: string): Promise<ExtractedSymbol[]> {
...
const PHP_PATTERNS: { kind: ExtractedSymbol["kind"]; regex: RegExp }[] = [
	...
	},
	{
		kind: "method",
		regex:
			/^\s*(?:public|protected|private)\s+(?:static\s+)?function\s+(\w+)\s*\(([^)]*)\)\s*(?::\s*([^\n{]+))?/gm,
	},
	{
		kind: "function",
...
export async function extractSymbolsFromFiles(
	...
	logger.debug(
		{ fileCount: filePaths.length, symbolCount: allSymbols.length },
		"Symbols extracted",
	);
	return allSymbols;
}
