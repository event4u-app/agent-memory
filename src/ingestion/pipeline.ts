...
import { scanFiles, type FileScannerOptions } from "./scanners/file-scanner.js";
import { readDocs, type DocReaderOptions } from "./scanners/doc-reader.js";
import { readGitCommits, type GitReaderOptions } from "./scanners/git-reader.js";
  ...
  async run(options: PipelineOptions): Promise<IngestionResult> {
    ...
    if (options.guardOptions) {
      ...
      if (!guardResult.allowed) {
        ...
        return { totalCandidates: 0, createdEntries: 0, contradictionsDetected: 0, guardBlocked: true, entryIds: [] };
      }
    }

    // 2. Run scanners in parallel
    const scannerResults = await Promise.all([
      !skip.has("files") ? scanFiles({ root: options.root, repository: options.repository, maxFiles: options.maxFiles }) : [],
      !skip.has("docs") ? readDocs({ root: options.root, repository: options.repository }) : [],
      !skip.has("git") ? readGitCommits({ root: options.root, repository: options.repository, maxCommits: options.maxCommits, since: options.since }) : [],
    ]);
