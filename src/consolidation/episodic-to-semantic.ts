...
export class EpisodicToSemanticExtractor {
	constructor(private readonly entryRepo: MemoryEntryRepository) {}
	 ...
	 */
	async extract(episodicEntry: MemoryEntry, repository: string): Promise<SemanticExtractionResult> {
	...
	private extractFacts(
		entry: MemoryEntry,
	): Omit<CreateEntryInput, "scope" | "consolidationTier" | "createdBy" | "createdInTask">[] {
		const facts: Omit<
			CreateEntryInput,
			"scope" | "consolidationTier" | "createdBy" | "createdInTask"
		>[] = [];
		...
		const bugPatterns = [/(?:bug|issue|fix|fixed|problem)[:\s]+(.+)/gi, /caused by (.+)/gi];
