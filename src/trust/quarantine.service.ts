...
import { MIN_EVIDENCE_COUNT, TRUST_SCORE_CAP_SINGLE_EVIDENCE } from "../types.js";
...
import { calculateTrustScore, calculateExpiryDate } from "./scoring.js";
...
export class QuarantineService {
  ...
  async validateEntry(entryId: string, triggeredBy = "system"): Promise<ValidationSummary> {
    ...
    const results = await Promise.all(
      this.validators.map((v) => v.validate(entry, evidence)),
    );
