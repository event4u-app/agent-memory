...
export interface DuplicateGroup {
  ...
  entries: { id: string; title: string; trustScore: number; accessCount: number }[];
  /** Overlap reason */
  reason: string;
}
...
export async function findDuplicates(sql: postgres.Sql): Promise<DuplicateGroup[]> {
  ...
  for (const row of rows) {
    ...
    if (!groups.has(key)) {
      groups.set(key, {
        entries: [
          { id: row.id_a as string, title: row.title_a as string, trustScore: row.score_a as number, accessCount: row.access_a as number },
          { id: row.id_b as string, title: row.title_b as string, trustScore: row.score_b as number, accessCount: row.access_b as number },
        ],
...
export async function mergeDuplicates(
  ...
  logger.info(
    { survivorId: survivor.id, mergedCount: toMerge.length, evidenceTransferred },
