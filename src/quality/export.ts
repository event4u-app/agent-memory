...
export async function exportSnapshot(
	...
	const statusFilter = options.includeArchived ? sql`` : sql`WHERE trust_status != 'archived'`;
	...
	const snapshot: MemorySnapshot = {
		...
		stats: {
			...
			byStatus: Object.fromEntries(
				statusCounts.map((r) => [r.trust_status as string, r.count as number]),
			),
...
export function diffSnapshots(
	...
	const changed = newer.entries
		.filter((e) => {
			...
			return (
				old &&
				(old.trustStatus !== e.trustStatus ||
					old.trustScore !== e.trustScore ||
