...
export async function calculateMetrics(sql: postgres.Sql): Promise<QualityMetrics> {
  ...
  `;
	const statusDistribution = Object.fromEntries(
		statusRows.map((r) => [r.trust_status as string, r.count as number]),
	);
  ...
  `;
	const staleByImpact = Object.fromEntries(
		staleRows.map((r) => [r.impact_level as string, r.count as number]),
	);
