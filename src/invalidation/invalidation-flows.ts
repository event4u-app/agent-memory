...
export async function softInvalidate(
  ...
  if (from === "stale" || from === "rejected" || from === "poisoned") {
    return { entryId, fromStatus: from, toStatus: from, reason: "Already invalidated", action: "skipped" };
  }

  if (!isValidTransition(from, "stale")) {
    return { entryId, fromStatus: from, toStatus: from, reason: `Cannot stale from ${from}`, action: "skipped" };
  }
  ...
  return { entryId, fromStatus: from, toStatus: "stale", reason, action: "soft" };
}
...
export async function hardInvalidate(
  ...
  if (from === "invalidated" || from === "rejected" || from === "poisoned") {
    return { entryId, fromStatus: from, toStatus: from, reason: "Already invalidated/rejected/poisoned", action: "skipped" };
  }

  if (!isValidTransition(from, "invalidated")) {
    return { entryId, fromStatus: from, toStatus: from, reason: `Cannot invalidate from ${from}`, action: "skipped" };
  }

  await entryRepo.transitionStatus(entryId, "invalidated", `Hard invalidation: ${reason}`, triggeredBy);

  logger.warn({ entryId, from, reason }, "Hard invalidation applied");

  return { entryId, fromStatus: from, toStatus: "invalidated", reason, action: "hard" };
}
