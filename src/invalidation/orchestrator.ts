...
import { readGitDiff, readGitDiffSince, type DiffResult } from "./git-diff.js";
import { matchFileWatches, matchSymbolWatches, type WatchMatch } from "./watchers.js";
import { detectDrift } from "./semantic-drift.js";
import { softInvalidate, hardInvalidate } from "./invalidation-flows.js";
...
export class InvalidationOrchestrator {
  ...
  async run(options: InvalidationRunOptions): Promise<InvalidationRunResult> {
    ...
    if (diff.changes.length === 0) {
      return { diff: { filesChanged: 0, fromRef: diff.fromRef, toRef: diff.toRef }, watchMatches: 0, softInvalidated: 0, hardInvalidated: 0, driftDetected: 0, skipped: 0 };
    }
    ...
    for (const entryId of affectedEntryIds) {
      ...
      if (hasSymbolMatch) {
        ...
        if (drift.shouldInvalidate) {
          driftCount++;
          await hardInvalidate(entryId, `Semantic drift: ${drift.driftedSymbols.map((s) => s.symbolName).join(", ")}`, this.entryRepo);
          hardCount++;
          continue;
        }
      }
      ...
      if (hasDeletedFile) {
        await hardInvalidate(entryId, `Watched file deleted: ${matches.find((m) => m.change.isDeleted)?.matched}`, this.entryRepo);
        hardCount++;
    ...
    logger.info(
      { filesChanged: diff.changes.length, matches: allMatches.length, soft: softCount, hard: hardCount, drift: driftCount },
      "Invalidation run complete",
    );

    return {
      diff: { filesChanged: diff.changes.length, fromRef: diff.fromRef, toRef: diff.toRef },
  ...
  private mapRow(row: postgres.Row): MemoryEntry {
    return {
      id: row.id, type: row.type, title: row.title, summary: row.summary, details: row.details,
      scope: typeof row.scope === "string" ? JSON.parse(row.scope) : row.scope,
      impactLevel: row.impact_level, knowledgeClass: row.knowledge_class,
      consolidationTier: row.consolidation_tier,
      trust: { status: row.trust_status, score: row.trust_score, validatedAt: row.validated_at ? new Date(row.validated_at) : null, expiresAt: new Date(row.expires_at) },
      embeddingText: row.embedding_text, embedding: row.embedding,
      accessCount: row.access_count, lastAccessedAt: row.last_accessed_at ? new Date(row.last_accessed_at) : null,
      createdBy: row.created_by, createdInTask: row.created_in_task,
