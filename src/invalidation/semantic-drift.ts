import { extractSymbols, type ExtractedSymbol } from "../ingestion/scanners/symbol-extractor.js";
...
export async function detectDrift(
  ...
  if (allTracked.size === 0) {
    return { entryId: entry.id, driftedSymbols: [], driftScore: 0, shouldInvalidate: false };
  }
  ...
  for (const trackedName of allTracked) {
    ...
    const symbolName = trackedName.includes("::")
      ? trackedName.split("::").pop()!
      : trackedName;
    ...
    if (!current) {
      ...
      drifted.push({
        ...
        filePath: storedEvidence ? findFileForEvidence(storedEvidence, watchedFiles) : watchedFiles[0] ?? "unknown",
        kind: "removed",
