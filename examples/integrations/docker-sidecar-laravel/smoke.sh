#!/usr/bin/env bash
# Smoke test for the docker-sidecar-laravel integration template.
#
# Static-only: validates the copy-paste template without booting the
# stack. Full end-to-end boot lives in `examples/laravel-sidecar/`,
# the backing reference project this integration distills.
#
...
#   4. Every `memory <subcommand>` in MemoryService.example.php is a
#      real CLI command (scraped from `memory --help`). Catches CLI
#      renames/removals before stale templates reach users.
...
host_code="MemoryService.example.php"
...
#
# `docker compose config` is authoritative: it resolves variables,
# merges extends, validates schema. Using it instead of a raw YAML
# parse catches far more shape bugs than `python3 -c 'yaml.safe_load'`.
...
#
# Without it, the `depends_on: service_healthy` guard on the app
# service has nothing to wait on — the template's core teaching point
# silently breaks.
...
#
# MemoryService uses `$this->run(['<subcommand>', ...])` — the subcommand
# is always the FIRST string literal inside the array passed to run().
# We collapse newlines with tr so multi-line run([\n  'ingest', ...])
# invocations become searchable on a single logical line, then anchor
# on `->run([<optional-ws>'cmd'`. This avoids false positives from prose
# in doc comments (e.g. "agent-memory as a sidecar" → fake 'as' cmd).
#
# `|| true` swallows grep's exit-1-on-no-match so `set -e` / pipefail
# don't kill the script before we can emit the clear error below.

# The `--` separator before the pattern prevents BSD grep on macOS from
# parsing the leading `->` as option flags.
cmds=$(tr '\n' ' ' < "${host_code}" \
  | grep -oE -- "->run\([[:space:]]*\[[[:space:]]*'[a-z][a-z_-]+'" \
  | sed -E "s|.*'([^']+)'\$|\\1|" \
  | sort -u \
  || true)

if [ -z "${cmds}" ]; then
  echo "❌  ${host_code}: no ->run(['<cmd>']) invocations found"
  echo "    Expected pattern: \$this->run(['health']) or \$this->run(['ingest', ...])"
  exit 1
fi
...
echo ""
echo "✨  docker-sidecar-laravel integration smoke passed"
