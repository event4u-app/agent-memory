...
>
> **Pre-spike dry-run:** Local Compose validation completed 2026-04-27 — see [`team-memory-dryrun-results.md`](team-memory-dryrun-results.md). Outcome: pass; one resolved finding (GHCR `:latest` reserved for v* releases — default tag now `main`, plus a one-time package-visibility step in [`deploy/team-memory/operator-setup.md` §4](../../deploy/team-memory/operator-setup.md#4--ghcr-package-visibility-one-time)). Use [`scripts/team-memory-smoketest.sh`](../../scripts/team-memory-smoketest.sh) for the Step 3 acceptance round-trip.
